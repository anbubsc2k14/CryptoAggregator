...
st.caption("Compare SIP vs Agentic RSI-based Smart DCA")
...
if run:
    try:
        summary, sip, agent = run_backtest(
            symbol=symbol,
            years=int(years),
        ...
        ax.plot(agent.value_series.index, agent.value_series.values, label='Agentic Smart DCA', linewidth=1.5)
...
- SIP buys run on first trading day each month; Agentic buys evaluated daily at close.
""")
